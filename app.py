...
import base64
from fetcher import smart_fetch, get_missing_data_points, load_master_csv, MASTER_CSV
...
def get_master_csv() -> str:
    return os.path.join(get_export_dir(), "fog_master.csv")
...
# Preferred master CSV — created/updated by the in-app fetcher
_MASTER_CSV = get_master_csv()
...
def recompute_combined(scores_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "PlantScore" in df.columns:
        combined_raw += df["PlantScore"] * st.session_state.weight_plant
        total_weight += st.session_state.weight_plant
    if "CloudScore" in df.columns:
        combined_raw += df["CloudScore"] * st.session_state.weight_cloud
        total_weight += st.session_state.weight_cloud
    if "ContrastScore" in df.columns:
        combined_raw += df["ContrastScore"] * st.session_state.weight_contrast
...
def load_fog_csv(csv_path: str, mtime: float) -> pd.DataFrame:
...
def get_fog_df() -> pd.DataFrame | None:
    if st.session_state.selected_csv and os.path.exists(st.session_state.selected_csv):
        ...
        return load_fog_csv(st.session_state.selected_csv, mtime)
    return None
...
def switch_hunt(new_hunt: str):
    ...
    st.session_state.selected_csv = get_master_csv() if os.path.exists(get_master_csv()) else None
    invalidate_scores()
...
with st.sidebar:
    ...
    if all_csvs:
        csv_names   = [os.path.basename(c) for c in all_csvs]
        default_idx = 0  # fog_master.csv is always first when it exists
        sel_name = st.selectbox("Active fog CSV", csv_names,
    ...
    with st.expander("🌿 iNaturalist Plant Settings", expanded=False):
        ...
        # 5. Color map by selection
        color_options = ["Combined Score", "Fog Match Rate"]
        if st.session_state.selected_plants:
            color_options.append("Plant Proximity")
        if st.session_state.cloud_obs_list:
            color_options.append("Cloud Differential Score")
        if st.session_state.contrast_obs_list:
            color_options.append("High-Contrast Cloud Score")

        new_color_by = st.selectbox(
            "Color grid points by",
            color_options,
            index=color_options.index(st.session_state.map_color_by) if st.session_state.map_color_by in color_options else 0,
            help="Choose how the spatial grid circles are colored."
        )
        if new_color_by != st.session_state.map_color_by:
            st.session_state.map_color_by = new_color_by
            st.rerun()
    ...
    with st.expander("🖼️ Map Image Overlays", expanded=False):
        ...
        for idx, overlay in enumerate(st.session_state.map_overlays):
            ...
            if vis:
                n_lat = st.number_input("Center Lat", value=overlay["lat"], step=0.01, format="%.4f", key=f"ov_lat_{idx}")
                n_lon = st.number_input("Center Lon", value=overlay["lon"], step=0.01, format="%.4f", key=f"ov_lon_{idx}")
                col_s1, col_s2 = st.columns(2)
                with col_s1:
                    n_w = st.number_input("Width (km)", value=float(overlay["width"]), step=1.0, key=f"ov_w_{idx}")
                with col_s2:
                    n_h = st.number_input("Height (km)", value=float(overlay["height"]), step=1.0, key=f"ov_h_{idx}")
                ...
                if (n_lat != overlay["lat"] or n_lon != overlay["lon"] or n_w != overlay["width"] or 
                    n_h != overlay["height"] or n_rot != overlay["rotation"] or n_op != overlay["opacity"]):
                    overlay["lat"] = n_lat
                    overlay["lon"] = n_lon
                    overlay["width"] = n_w
                    overlay["height"] = n_h
...
with tab3:
    ...
    if fog_df is None:
        ...
        if scores is None or scores.empty:
            ...
            # Map
            color_col = "MatchRate"
            color_title = "Match Rate"
            color_scale = SCORE_COLORSCALE
            range_color = [0.0, 1.0]

            if st.session_state.map_color_by == "Combined Score" and ("PlantScore" in scores.columns or "CloudScore" in scores.columns or "ContrastScore" in scores.columns):
                color_col = "CombinedScore"
                color_title = "Combined Suitability"
                color_scale = "Plotly3"
                range_color = [0.0, float(scores["CombinedScore"].max()) if not scores["CombinedScore"].empty and scores["CombinedScore"].max() > 0 else 1.0]
            elif st.session_state.map_color_by == "Plant Proximity" and "PlantScore" in scores.columns:
                color_col = "PlantScore"
                color_title = "Plant Score"
                color_scale = "Viridis"
                range_color = [0.0, 1.0]
            elif st.session_state.map_color_by == "Cloud Differential Score" and "CloudScore" in scores.columns:
                color_col = "CloudScore"
                color_title = "Cloud Differential"
                color_scale = "Blues"
                range_color = [0.0, 1.0]
            elif st.session_state.map_color_by == "High-Contrast Cloud Score" and "ContrastScore" in scores.columns:
                color_col = "ContrastScore"
                color_title = "High-Contrast Cloud"
                color_scale = "Oranges"
                range_color = [0.0, 1.0]
