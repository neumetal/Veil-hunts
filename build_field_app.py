...
# 4b. Map Settings (point opacity slider) — always present in field app
out.append('\n')
out.append('    with st.expander("🗺️ Map Settings", expanded=False):\n')
out.append('        if "map_opacity" not in st.session_state:\n')
out.append('            st.session_state.map_opacity = 80\n')
out.append('        _nt = st.slider(\n')
out.append('            "Point Opacity (%)", 5, 100,\n')
out.append('            int(st.session_state.map_opacity), step=5,\n')
out.append('            key="fa_trans",\n')
out.append('            help="Higher = solid grid points. Lower = more see-through. Image overlay opacity is controlled separately."\n')
out.append('        )\n')
out.append('        if _nt != st.session_state.map_opacity:\n')
out.append('            st.session_state.map_opacity = _nt\n')
out.append('            st.rerun()\n')
out.append('\n')
...
out.append('                st.rerun()\n')
out.append('            if vis:\n')
out.append('                n_op_pct = st.slider("Overlay Opacity (%)", 0, 100, int(round(float(overlay["opacity"]) * 100)), step=5, key=f"ov_op_{idx}")\n')
out.append('                n_op = n_op_pct / 100.0\n')
